        ...
        for (int i = 1; i < Math.pow(2, N) / 2; i++) {
            ...
            for (int j = 0; j < start.size(); j++) {
                for (int k = j; k < start.size(); k++) {
                    if (j != k)
                        startAbil = startAbil + person[start.get(k)][start.get(j)] + person[start.get(j)][start.get(k)];
                }
            }
            ...
            for (int j = 0; j < link.size(); j++) {
                for (int k = j; k < link.size(); k++) {
                    if (j != k)
                        linkAbil = linkAbil + person[link.get(j)][link.get(k)] + person[link.get(k)][link.get(j)];
                }
            }
